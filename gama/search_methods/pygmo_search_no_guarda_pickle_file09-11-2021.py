# -*- coding: utf-8 -*-
"""
Created on Tue Nov  9 16:12:40 2021
...
def pygmo_serach(
    ...
    print("Successive halving paper", iters)
    successive = False
    ...
    if not successive:
        ...
        path = path + "/pickle_gama/" + "list_successive_halving.pkl"
        list_successive_halving = pickle.load(open(path, "rb"))
        m = -len(list_successive_halving)
        number_promoted_individuals = math.ceil(number_of_configurations*(reduction_factor**(-m)))
        if number_promoted_individuals < 10: number_promoted_individuals = 10
        ...
        if os.path.isfile(path):
            ...
            output = top(elementos=number_promoted_individuals)
            if len(output) == 0:
                path_use = os.getcwd()
                path = path_use.replace(os.sep, '/')
                path = path + "/pickle_gama/"
