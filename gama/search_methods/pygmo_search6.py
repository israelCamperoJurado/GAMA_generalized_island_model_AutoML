# -*- coding: utf-8 -*-
"""
Created on Sun Aug 22 19:33:41 2021
...
class AutoMLProblem:
    ...
    contador = 0
    # def __init__(self, ops, start_candidates, max_n_evaluations, n_evaluated_individuals):
    def __init__(self, ops):
        # self.max_n_evaluations = max_n_evaluations
        # self.n_evaluated_individuals = n_evaluated_individuals
        # self.start = start_candidates
        self.operator = ops
        self.output = []
    # Define objectives
    def fitness(self, x):
        ...
        with AsyncEvaluator() as async_:
            # for individual in self.start:
            #     async_.submit(self.operator.evaluate, individual)
            ...
            # while (self.max_n_evaluations is None) or (
            #     self.n_evaluated_individuals < self.max_n_evaluations
            # ):
            future = self.operator.wait_next(async_)
            if future.exception is None:
                individual_prototype = future.result.individual
                print("Individuo evaluado en PyGMO Search ", individual_prototype)
                AutoMLProblem.save_ind.append(individual_prototype)
            self.output = AutoMLProblem.save_ind
            #f1 = individual_prototype.fitness.values[0]
...
def pygmo_serach(
    ...
    # prob = pg.problem(AutoMLProblem(ops, start_candidates, max_n_evaluations, n_evaluated_individuals))
    ...
    ## ++++++++++++++++++++++++++++
    # try:
    #     # class_support = AutoMLProblem(ops, start_candidates, max_n_evaluations, n_evaluated_individuals)
    ...
    # # fitness_of_island_champion = archi.get_champions_f()
    # x_of_island_champion = archi.get_champions_x()
    algo = pg.algorithm(pg.de(gen = 30))
    prob = pg.problem(AutoMLProblem(ops))
    # prob = pg.problem(AutoMLProblem(ops, start_candidates, max_n_evaluations, n_evaluated_individuals))
