...
class RandomForestTry(BaseSearch):
    ...
    def search(self, operations: OperatorSet, start_candidates: List[Individual]):
        inteto5 = exampleF(operations, ind)
        print("imprimir intento 5", inteto5)
...
def exampleF(ops: OperatorSet, ind1: Individual) -> Individual:
    with AsyncEvaluator() as async_:
        async_.submit(ops.evaluate, ind1)
        future = ops.wait_next(async_)
        if future.exception is None:
            individualjuego = future.result.individual
        return individualjuego
