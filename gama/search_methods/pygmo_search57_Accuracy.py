# -*- coding: utf-8 -*-
"""
Created on Tue Sep 14 09:51:34 2021
...
def pygmo_serach(
    ...
    print("len start_candidates", len(start_candidates))
    successive = False
    ...
    if not successive:
        ...
        for individual in start_candidates:
            ...
            print("The individual evaluate in start candidates is", new_ind)
        ...
        if os.path.isfile(path):
            ...
            new_warm_start = new_warm_start + support_to_successive
        else:
            print("Usaré los individuos")
            new_warm_start = top(elementos=10)
            if len(new_warm_start) == 0:
                path_use = os.getcwd()
                path = path_use.replace(os.sep, '/')
                path = path + "/pickle_gama/"
                for root, dirs, files, in os.walk(path):
                        ...
                        if file.endswith(".pkl"):
                            ...
                            #new_lista = pickle.load(open(new_f_path, "rb"))
                                ...
                                try:
                                    ...
                                    new_lista = []
                                new_warm_start = new_warm_start + new_lista
        ...
        f_vectors = [new_individual.fitness.values[0] for new_individual in new_warm_start]            
        x_vectors = []
        for i in new_warm_start:
            instance_individual_to_vectors = IndividuoVector()
            new_vector = instance_individual_to_vectors(i)
            x_vectors.append(new_vector)
        ...
        #We keep the best individuals in all the iterations
        path_use = os.getcwd()
        path = path_use.replace(os.sep, '/')
        path = path + "/pickle_gama/" + "best_individuals.pkl"
        with open(path, 'wb') as f:
            pickle.dump(new_warm_start, f)
