...
import stopit # To control the time
...
class SearchPygmo(BaseSearch):
    ...
    def search(self, operations: OperatorSet, start_candidates: List[Individual]):
        self.output = pygmo_serach(
            operations, self.output, start_candidates, self.path, self.timeout, **self.hyperparameters
        )
...
def pygmo_serach(
    ...
    def get_job(): # In this function we are going to receive an individual and a rung
        for rung, individuals in list(rung_individuals.items())[1:]:
            # This is not in the paper code but is derived from fig 2b
            n_to_promote = math.floor(len(individuals) / reduction_factor) # How many individuals are going to participate in the next rung
            if n_to_promote - len(promoted_individuals[rung]) > 0:
                ...
                if len(not_promoted) > 0:
                    to_promote = max(not_promoted, key=lambda i: i[0]) # choose the one to promote
    ...
    # print("timeout pygmo search", timeout)
    # new_time_out = int(timeout/len(rungs))
    # print("new_time_out pygmo search", new_time_out)
    for rung in rungs:
        # with stopit.ThreadingTimeout(new_time_out):
        ops.evaluate = partial(ops.evaluate, subsample=rung_resources[rung])
        evaluate = partial(
            evaluate_on_rung, evaluate_individual=ops.evaluate, max_rung=max_rung
        )
