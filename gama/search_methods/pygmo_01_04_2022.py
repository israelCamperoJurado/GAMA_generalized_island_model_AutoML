...
def pygmo_serach(
    ...
    def get_job():  # In this function we are going to receive an individual and a rung
        for rung, individuals in list(rung_individuals.items())[1:]:
            ...
            n_to_promote = math.floor(
                len(individuals) / reduction_factor)  # How many individuals are going to participate in the next rung
    ...
    for rung in rungs:
        ...
        time_limit = (int(time_limit / len(
            rungs))) - 30  # 30 seconds of compensation for the first 50 individuals evaluated
        print("New time_limit inside pygmo class search", time_limit)

        with stopit.ThreadingTimeout(time_limit):
            print("Let's goooo")
            print("Let's goooo")
            print("Let's goooo")
            print("Let's goooo")
