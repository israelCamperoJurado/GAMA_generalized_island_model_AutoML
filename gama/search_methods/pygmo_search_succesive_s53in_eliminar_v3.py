# -*- coding: utf-8 -*-
"""
Created on Sun Sep 12 22:28:12 2021
...
def pygmo_serach(
    ...
    if not successive:
        ...
        if os.path.isfile(path):
            ...
            print("Voy a usar los individuos en la carpeta")
            path_use = os.getcwd()
            path = path_use.replace(os.sep, '/')
            path = path + "/pickle_gama/"
            for root, dirs, files, in os.walk(path):
                for file in files:
                    ...
                    if file.endswith(".pkl"):
                        ...
                        if (file=="list_successive_halving.pkl"):
                            print("TThis won't be take in consideration")
                        #new_lista = pickle.load(open(new_f_path, "rb"))
                        else:
                            try:
                                new_lista = pickle.load(open(new_f_path, "rb"))
        ...
        #current_population=final_lista # Esta es la linea que modifique
        print("Longitud final", len(current_population))
